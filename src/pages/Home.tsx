...
async function sendFeedback({ user_id, feedback, rating }: FeedbackParams) {
...
const FeedbackModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (params: FeedbackParams) => Promise<void>;
}) => {
  const [feedback, setFeedback] = useState("");
  const [rating, setRating] = useState(0);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const storedUser = LocalStorageManager.get("user") as StoredUser;

  const getUserId = () => {
    if (!storedUser) return "guest_user";
    if ("user_id" in storedUser) return storedUser.user_id;
    if ("guest_id" in storedUser) return storedUser.guest_id;
    return "guest_user";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ feedback, rating, user_id: getUserId() });
        ...
        <div className="text-center">
          ...
          <div className="mt-12 text-center">
            ...
            <button
              ...
              className="inline-flex items-center px-6 py-2 text-base font-medium rounded-md text-white bg-lime-600 hover:bg-lime-700 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-colors"
            >
              <svg
